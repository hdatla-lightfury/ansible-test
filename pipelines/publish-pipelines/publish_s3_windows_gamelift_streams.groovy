...
pipeline {
    parameters {
        ...
        string(name: 'S3_BUCKETNAME', defaultValue: 'titangamecast', description: 'Target S3 Bucket')
    }
    ...
    stages {
        ...
        stage('Set env variables') {
            steps {
                script {
                    ...
                    env.SUB_FOLDER_NAME = envVars['SUB_FOLDER_NAME']
            ...
            steps {
                script {
                    if(env.BUILD_CONFIG == "Test"  || env.BUILD_CONFIG == "Shipping"){
                        def folderToUpload = env.SUB_BUILD_FOLDER_PATH + "\\\\windows"
        ...
        stage('Upload to AWS S3') {
            steps {
                script {
                    def AwsOps = new AwsOperations(this, env)
                    if (params.S3_BUCKETNAME?.trim()) {
                        AwsOps.uploadToS3(params.S3_BUCKETNAME, env.SUB_FOLDER_NAME)
                    } else {
                        error("S3_BUCKETNAME parameter is missing!")
                    }
                }
            }
        }
            ...
            steps {
                script {
                    try {
                        ...
                        echo "Warning: Failed to delete env vars file - ${e.message}"
                    }
                }
            }
        }
